...
public abstract class AbstractaListRepositorio<T extends BaseEntity> implements OrdenablePaginableCrudRepositorio<T> {
    ...
    public T porId(Integer id) throws LecturaAccesoDatoException {
        if(id == null || id <= 0){
            throw new LecturaAccesoDatoException("Id inválido debe ser > 0");//lanzamos lectura
        }
        T resultado = null;
    ...
    public void crear(T t) throws EscrituraAccesoDatoException{
        ...
        if(this.dataSource.contains(t)){//si contiene el objeto (BaseEntity tiene el equals por el id)
            throw new RegistroDuplicadoAccesoDatoException("Error el objeto con id "
                    + t.getId() + " existe en el repositorio");//hereda de escrituraAccesoDatos -> es más expecifica
        }
